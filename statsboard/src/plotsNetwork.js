...
export function makePlotsNetwork(startTime, endTime, node, network) {
  ...
  // make calls to retrieve list of nodes and networks
  const call1 = fetch('https://ws.resif.fr/eidaws/statistics/1/nodes');
  const call2 = fetch('https://ws.resif.fr/eidaws/statistics/1/networks');
  Promise.all([call1, call2])
    .then(([response1, response2]) => Promise.all([response1.json(), response2.json()]))
    .then(([data1, data2]) => {
      const nodes = data1.nodes.map(node => node.name).sort();
      const networks = data2.networks.map(network => network.name).sort();
      ...
      function totalPlots() {
        ...
        function monthAndYearPlots(details = "month") {
          let url = null;
          if (details === "year") {
            url = `https://ws.resif.fr/eidaws/statistics/1/dataselect/public?start=${startTime}${endTime ? `&end=${endTime}` : ''}${node ? `&node=${node}` : ''}${network ? `&network=${network}` : ''}&level=network&details=year&format=json`;
          }
          else {
            url = `https://ws.resif.fr/eidaws/statistics/1/dataselect/public?start=${startTime}${endTime ? `&end=${endTime}` : ''}${node ? `&node=${node}` : ''}${network ? `&network=${network}` : ''}&level=network&details=month&format=json`;
          }
          fetch(url)
            .then((response) => {
              if (response.ok) {
                      ...
                      args: [
                        {
                          ...
                          name: barData.map(bar => bar.network),
                          type: 'bar'
                        },
                        {
                          title: 'Number of unique users per '+details,
        ...
        function mapPlots() {
          const url = `https://ws.resif.fr/eidaws/statistics/1/dataselect/public?start=${startTime}${endTime ? `&end=${endTime}` : ''}${node ? `&node=${node}` : ''}${network ? `&network=${network}` : ''}&level=network&details=country&format=json`;
          fetch(url)
            .then((response) => {
              if (response.ok) {
                return response.json();
            ...
            .then((data) => {
              // remove networks not included in the results
              networks = networks.filter(network => data.results.map(result => result.network).includes(network));
